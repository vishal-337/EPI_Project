...
rankings_path = os.path.join(OUT, "influencer_rankings.csv")
...
top_k = 5
top_influencers = rankings.head(top_k)["STATE_ABBREV"].tolist()
...
for sim in range(n_simulations):
    ...
    for year in years[1:]:
        newly_active = set()
        for node in active:
            if node in top_influencers and year >= 2008:
                continue
...
avg_res.to_csv(os.path.join(OUT, "intervention_results.csv"), index=False)
print("Intervention results saved to outputs/intervention_results.csv")
